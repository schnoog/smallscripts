...
desiredTemp = 30
maxTemp = 55 #At this temperature the fan will run at 100%
...
class FreqThread(threading.Thread):
        def __init__(self, queue, tempPin, rpmReadCycle, filepath, args=(), kwargs=None):
                ...
                GPIO.setup(self.tpin, GPIO.IN, pull_up_down=GPIO.PUD_UP)
                self.fpath = filepath
...
def changeDuty(): #calculates the needed PWM duty and changes it
        ...
        f=open("/dev/shm/pwmstats","w")
        f.write("actualTemp {:4.2f} TempDiff {:4.2f} pDiff {:4.2f} pwmDuty {:5.0f}".format(temp, diff, pDiff, pwmDuty))
        f.close()
